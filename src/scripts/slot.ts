...
export class Slot {
    ...
    constructor(domElement: HTMLElement, config: SlotConfig = {}) {
      ...
      spin(): Promise<void> {
        ...
        let isSoundOn = JSON.parse(window.localStorage.getItem("isSoundOn") || "true");
        if (isSoundOn) {
          SMSoundService.enable();
        ...
        return Promise.all(
          ...
          const isThala = this.checkFirstIndexIsSeven(this.nextSymbols);
          if (isThala) {
            SMSoundService.thalaSound.play(0.08);
            document.querySelector("#thalaDancing")?.classList.remove("hidden");
          }
        });
      }
